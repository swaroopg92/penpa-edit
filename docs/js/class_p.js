...
class Puzzle {
    ...
    show_pause_layer() {
        ...
        let textstring = PenpaText.get('pause_message') || "Paused\nClick on \"Start\"\nor \"F4\"";
    ...
    paste_handler(ev) {
        ...
        if (clipboard_data === "" || !clipboard_data) {
            let plain_text = ev.clipboardData.getData("text/plain");
            if (plain_text) {
                // Do not use trim() directly on plain_text to preserve leading empty cells (e.g. leading tabs)
                let lines = plain_text.replace(/\s+$/, '').split(/\r?\n/);
                if (lines.length > 0) {
                    let items = [];
                    let y = 0;
                    for (let line of lines) {
                        ...
                        for (let col of cols) {
                            ...
                            if (col !== "") {
                                let edit_mode = this.mode[this.mode.qa].edit_mode;
                                let style = this.mode[this.mode.qa][edit_mode][1];
                                items.push({ x: x, y: y, number: [col, style] });
                            }
                            x++;
                        }
                        y++;
                    ...
                    if (items.length > 0) {
                        clipboard_data = { gridtype: this.gridtype, items: items };
                    }
                }
            }
        }
